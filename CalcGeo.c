...
void searchLeaderboard(int targetScore) {
    FILE *file = fopen("leaderboard.txt", "r");
    if (file != NULL) {
        ...
        int result = binarySearch(scores, count, targetScore);
        if (result != -1) {
            int found = 0;
            ...
            if (!found) {
                printf("Score %d not found in the leaderboard.\n", targetScore);
            }
        } else {
            printf("Score %d not found in the leaderboard.\n", targetScore);
        }
...
int main() {
    ...
    do {
        ...
        printf("5. Search score on leaderboard\n");
        printf("6. Exit\n");
